...
function CorretorRankCard<T extends { id: number }>({
  ...
  const sorted = (items || []).slice().sort((a, b) => (getValue(b) ?? 0) - (getValue(a) ?? 0));
  const max = sorted.length > 0 ? (getValue(sorted[0]) ?? 0) || 1 : 1;
...
export default function Dashboard() {
  ...
  const pipelinePorCorretor = useMemo(() => {
    ...
    const ensure = (id: number, nome: string): Row => {
      ...
      else if (nome && nome.length > (map.get(id)!.nome?.length ?? 0)) map.get(id)!.nome = nome;
      return map.get(id)!;
    };
